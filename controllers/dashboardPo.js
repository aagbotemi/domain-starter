  ...
  count: async (req, res) => {
    try {
      ...
      const employCount2 = await db.beneficiaries.count({
        where: {
            [Op.and]: [{ partnerorganisationId: req.poId }, { employmentStatus: "employed" }]
          }
      });
      const employCount1 = await db.beneficiaries.count({
        where: {
            [Op.and]: [{ partnerorganisationId: req.poId }, { employmentStatus: "self employed" }]
          }
      });
      const unemployCount = await db.beneficiaries.count({
        where: {
            [Op.and]: [{ partnerorganisationId: req.poId }, { employmentStatus: "unemployed" }]
          }
      });
